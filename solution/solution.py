...
def run_assistant(
    ...
    while True:
        ...
        cost_stats = estimate_cost(full_prompt_text, reply, OPENAI_MODEL)
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        model_comparation = compare_models(prompt=prompt)
        model_comparation["prompt"] = prompt
        model_comparations.append(model_comparation)
    return model_comparations
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def truncate(text, max_len=40):
        ...
        return text_str.ljust(max_len)

    headers = ["Prompt", "GPT-4o Response", "Mini Response", "GPT-4o Latency", "Mini Latency"]
    header_str = " | ".join(truncate(h) for h in headers)
    separator = "-" * len(header_str)
    
    lines = [header_str, separator]
    
    for row in results:
        r1 = truncate(row.get("prompt", ""))
        r2 = truncate(row.get("gpt4o_response", ""))
        r3 = truncate(row.get("mini_response", ""))
        r4 = truncate(f"{row.get('gpt4o_latency', 0):.2f}s")
        r5 = truncate(f"{row.get('mini_latency', 0):.2f}s")
        lines.append(f"{r1} | {r2} | {r3} | {r4} | {r5}")
